...
export default function AboutUs() {
  ...
  useEffect(() => {
    ...
    // Observer for about text
    const textObserver = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          aboutTextRef.current.classList.add("animate-slide-in");
        }
      },
      { threshold: 0.5 }
    );
    ...
    if (aboutTextRef.current) {
      textObserver.observe(aboutTextRef.current);
    }
    ...
    return () => {
      ...
      if (aboutTextRef.current) {
        textObserver.unobserve(aboutTextRef.current);
      }
    };
  }, []);
  ...
  return (
    <section className="relative py-16 mb-12 overflow-hidden" id="AboutUs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h2
          ref={headingRef}
          className="text-5xl font-bold text-gray-900 mb-12 text-center opacity-0 transform translate-y-8"
        >
        ...
        <div className="relative mb-24">
          <div
            ref={aboutTextRef}
            className="relative bg-white p-8 rounded-lg shadow-lg z-10 opacity-0 transform translate-y-8"
          >
            <p className="text-lg text-justify text-gray-700">{aboutText}</p>
          </div>
